  ...
  while (1)
  {
    ...
    HAL_UART_Transmit(&huart1, (uint8_t*)"loop start\r\n", 12, 100);
		...
		while(expiretime>HAL_GetTick())
		{
			...
			uint32_t cc1_Flag=__HAL_TIM_GET_FLAG(&htim1,TIM_FLAG_CC1);
			uint32_t cc2_Flag=__HAL_TIM_GET_FLAG(&htim1,TIM_FLAG_CC2);
			if(cc2_Flag&&cc1_Flag)
			{
				success=1;
				break;
			}
		...
		if(success==1)
		{
			...
			if (ccr2 >= ccr1)
				...
				{
					diff = (65536 - ccr1) + ccr2;
				}
			float pulse = diff * 1e-6f;
//		float pulse=(ccr2-ccr1)*1e-6f;			
			float distance=340.0f*pulse/2.0f;
			len = sprintf(buf, "CCR1=%u CCR2=%u dist=%.2fcm\r\n", ccr1, ccr2, distance * 100);
    ...
    HAL_UART_Transmit(&huart1, (uint8_t*)"timeout!\r\n", 10, 100);  // 加上超时打印，方便调试
		...
		HAL_Delay(1000);  // 每1秒测一次，调试阶段看清楚数据
    /* USER CODE END WHILE */
