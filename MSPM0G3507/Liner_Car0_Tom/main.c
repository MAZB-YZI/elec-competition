...
#define TURN_HOLD   250           /* 直角最大持续时间 ms */
#define TURN_MIN    100           /* 直角最短保持 ms */
#define TURN_SPEED_H 1500         /* 直角转弯 PWM */

static volatile float   g_KP         = 2.0f;   /* 位置比例 */
static volatile float   g_KI         = 0.3f;   /* 位置积分 */
static volatile int16_t g_BASE_PWM   = 700;    /* 直行基准 PWM */
static volatile int16_t g_TURN_SPEED = 500;    /* 蓝牙可调转弯速度 */
...
void CTRL_TIMER_INST_IRQHandler(void)
{
    ...
    case DL_TIMER_IIDX_LOAD: {
        ...
        int16_t pos = ( 120)*s[0] + ( 70)*s[1] + ( 40)*s[2] + ( 30)*s[3]
                    + (-30)*s[4] + (-40)*s[5] + (-70)*s[6] + (-120)*s[7];

        /* 转向增量限幅: 每次最多变 ±100, 防电机猛跳 */
        int16_t steer = 0;
        int16_t new_steer = 0;
        ...
        if (st == NORMAL) {
            ...
            g_turn_ticks++;
            /* 至少转够 TURN_MIN 才检测退出; 超时也退出 */
            if ((g_turn_ticks > (TURN_MIN/5) && turn_done(raw))
        ...
        if (st == TURN_L) {
            ...
            if (s[0] || s[1] || s[2] || s[3] || s[4] || s[5] || s[6] || s[7]) {
                /* 中间两路(3,4)全黑 → 直走, 否则立刻修正 */
                if (s[3] && s[4])
                    steer = 0;
                else
                    steer = PID_Compute(&g_pid, 0, pos, 0.005f);
                g_last_steer = steer;
                g_lost_cnt   = 0;
            } else {
                /* 真丢线 */
        ...
        if (g_new_data) {
            ...
            if (++tick % 100 == 0) {
                ...
                if (yaw_int < 0) {
                    ...
                    OLED_ShowNum(24, 52, (uint32_t)(-yaw_int) / 10, 3, 12);
                } else {
                    OLED_ShowString(16, 52, "+", 12);
                    OLED_ShowNum(24, 52, (uint32_t)yaw_int / 10, 3, 12);
                }
                OLED_ShowString(48, 52, ".", 12);
                OLED_ShowNum(56, 52, (uint32_t)(yaw_int > 0 ? yaw_int : -yaw_int) % 10, 1, 12);
