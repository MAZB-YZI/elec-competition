...
static volatile bool     g_bno_ok;       /* BNO055 最近一次读取是否成功 */
static volatile uint32_t g_bno_samples;  /* BNO055 成功采样计数 */
...
void CTRL_TIMER_INST_IRQHandler(void)
{
    ...
    case DL_TIMER_IIDX_LOAD: {
        ...
        /* ---- Route 状态机优先 (弧线循迹在 route_fsm.c 内读灰度) ---- */
        if (Route_Update5ms(yaw)) {
            g_raw = raw;
            g_pos = GraySensor_GetPosition(raw);
            g_steer = 0;
            g_yaw = yaw;
            g_new_data = true;
            break;
...
int main(void)
{
    ...
    OLED_ShowBootStatus("BNO OK");
    delay_ms(200);          /* 等 BNO055 NDOF 姿态稳定 */
    MPU6050_ResetYaw();     /* 按 RESET 后自动清零 yaw */
    ...
    while (1) {
        ...
        if ((now - last_bno_ms) >= 10U) {
            uint32_t elapsed = now - last_bno_ms;
            last_bno_ms = now;
            bool ok = MPU6050_UpdateYawOnly((float)elapsed / 1000.0f);
            g_bno_ok = ok;
            if (ok) {
                g_bno_samples++;
            }
        ...
        if (g_new_data) {
            ...
            if (++tick % 100 == 0) {
                ...
                OLED_ShowString(76, 18, g_bno_ok ? "BNO" : "BER", 12);
                /* 显示全局累计圈数（陀螺仪持续累加） */
