/**
 * main.c �?PID 巡线 + L 形直�?+ 蓝牙调参
 *
 * 状�? NORMAL / TURN_L / TURN_R
...
/* ========== 默认参数 (蓝牙可改) ========== */
#define DEAD_ZONE   3             /* 位置死区: ±3 内不�?*/
#define LOST_MS     500           /* 丢线超时 ms */
#define TURN_TARGET 90.0f        /* 直角目标角度 (�? */
#define TURN_TIMEOUT 2000         /* 直角超时保护 ms */
#define TURN_SPEED_H 1500         /* 直角转弯 PWM */
#define STEER_SLEW_STEP 75        /* 5ms 内最大转向变化，提高响应速度 */
#define TURN_COOLDOWN_TICKS 40    /* 直角退出后冷却 200ms，防止二次触�?*/
#define TURN_FORWARD_PULSES 400   /* 转弯前前进编码器脉冲�?*/
#define CONTROL_DT 0.005f         /* 控制周期 5ms */
#define LEFT_ENCODER_DIR   1      /* 左轮编码器方向系�?*/
#define RIGHT_ENCODER_DIR -1     /* 右轮编码器方向系数，右轮安装反向 */
#define SPEED_LOOP_ENABLE 0       /* 0=开环测试编码器�?=启用速度PID */
...
static volatile float   g_KP         = 1.8f;   /* 位置比例，中等响�?*/
static volatile float   g_KI         = 0.0f;   /* 位置积分，先关掉 */
static volatile float   g_KD         = 0.0f;   /* 位置微分，默认关�?*/
static volatile int16_t g_BASE_PWM   = 800;    /* 直行基准 PWM */
...
static volatile int16_t  g_steer;        /* 当前转向修正�?*/
static volatile float    g_yaw;          /* 当前航向�?*/
static volatile State_t  g_state;        /* 状态机: NORMAL/TURN_L/TURN_R */
static volatile bool     g_new_data;     /* ISR 新数据标�?*/
...
static PID_t      g_speed_pid_l;         /* 左轮速度�?*/
static PID_t      g_speed_pid_r;         /* 右轮速度�?*/
static PID_t      g_turn_angle_pid;      /* 直角转弯角度�?*/
static float      g_target_yaw;          /* 目标航向�?(�? */
static int16_t    g_last_steer;          /* 上次转向�?丢线保持�? */
static int16_t    g_last_pos_ctrl;       /* 上次位置误差，用�?D �?*/
static uint32_t   g_lost_cnt;            /* 丢线持续计数 */
static uint32_t   g_turn_ticks;          /* 转弯持续 5ms 计数 */
static uint32_t   g_turn_confirm_ticks;  /* 直角退出确认计�?*/
static int32_t    g_last_enc_l, g_last_enc_r; /* 编码器上次�?*/
static volatile int16_t g_speed_l, g_speed_r; /* 编码器速度 脉冲/�?*/
static uint32_t   g_turn_cooldown;            /* 直角退出冷却计�?*/
static float      g_turn_start_yaw;           /* 直角起始航向 */
static State_t    g_pending_turn;             /* 等待中的转弯方向 */
static uint32_t   g_all_white_cnt;            /* 全白持续计数 */
static int32_t    g_turn_start_l, g_turn_start_r; /* 转弯等待编码器起始�?*/
static float      g_accumulated_angle;        /* 当前转弯累计角度 */
static float      g_last_yaw_for_accum;       /* 上次yaw，用于计算增�?*/
...
static bool MPU6050_Bringup(void)
{
    ...
    OLED_ShowString(0, 20, "HW I2C INIT", 12);
    OLED_Refresh();
    ...
    if (!MPU6050_Init()) {
        ...
        OLED_ShowNum(36, 16, MPU6050_GetWhoAmI(), 3, 12);
        OLED_Refresh();
        return false;
    }
...
static void MPU6050_Service(void)
{
    ...
    last_update_ms = now;
    g_mpu_read_ok = MPU6050_UpdateYawOnly((float) elapsed_ms / 1000.0f);
}
/* ================================================================
 *  TIMG6 ISR: 5ms 控制
 * ================================================================ */
    ...
    case DL_TIMER_IIDX_LOAD: {
        ...
        if (st == NORMAL) {
            ...
            if (g_accumulated_angle > 60.0f) {
                if (st == TURN_R && black(raw, 1))  /* 右转: 左边第二个看到黑�?*/
                    sensor_exit = true;
                if (st == TURN_L && black(raw, 6))  /* 左转: 右边第二个看到黑�?*/
                    sensor_exit = true;
            }
...
int main(void)
{
    ...
    g_target_yaw = 0.0f;
    g_total_angle = 0.0f;
    g_last_yaw_for_total = MPU6050_GetYaw();  /* 记录初始航向 */
    BT_Init();
